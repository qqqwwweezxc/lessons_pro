...
def list_of_hastags(text: str) -> list:
...
print(list_of_hastags(hastags))

# Напишіть функцію, яка перетворює дати з формату DD/MM/YYYY у формат YYYY-MM-DD.

def date_convertate(date: str) -> str:
    """Function convertate date from formate DD/MM/YYYY to YYYY/MM/DD"""
    day, month, year = date.split("/")
    return f"{year}-{month}-{day}"

print(date_convertate("24/10/2000"))
...
def find_all_ippv4(text: str) -> list:
...
def analyze_log_file(log_filename: str) -> str:
    """
    Function analyzes the web server log file and displays statistics on the number of requests from different IP addresses
    """
